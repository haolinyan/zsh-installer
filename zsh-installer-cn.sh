...
# 在Ubuntu和Alibaba Cloud Linux系统上安装zsh、oh-my-zsh、其插件和最新Node.js的脚本（国内优化版）
# 支持apt、yum和dnf包管理器
...
install_ohmyzsh() {
    ...
    if [ -d "$HOME/.oh-my-zsh" ]; then
        ...
        echo "请手动安装oh-my-zsh：sh -c \"$(curl -fsSL https://gitee.com/mirrors/oh-my-zsh/raw/master/tools/install.sh)\""
        return 1
    fi
}
...
# 安装zsh-syntax-highlighting插件（使用国内镜像）
install_zsh_syntax_highlighting() {
    ...
    echo "安装zsh-syntax-highlighting插件（使用国内镜像）..."
    git clone https://gitee.com/mirrors/zsh-syntax-highlighting.git "$ZSH_SYNTAX_HIGHLIGHTING_DIR"
...
# 安装zsh-autosuggestions插件（使用国内镜像）
install_zsh_autosuggestions() {
    ...
    echo "安装zsh-autosuggestions插件（使用国内镜像）..."
    git clone https://gitee.com/mirrors/zsh-autosuggestions.git "$ZSH_AUTOSUGGESTIONS_DIR"
...
# 安装最新的Node.js（使用国内镜像源）
install_nodejs() {
    echo "安装最新的Node.js（使用国内镜像源）..."
    
    # 使用国内镜像源安装最新的LTS版本
    # 根据包管理器类型安装Node.js
    if [[ "$INSTALL_CMD" == *"apt"* ]]; then
        # Ubuntu/Debian系统
        echo "添加NodeSource国内镜像源..."
        $SUDO apt-get update
        ...
        # 安装最新的LTS版本
        NODE_MAJOR=20
        echo "deb [signed-by=/etc/apt/keyrings/nodesource.gpg] https://mirrors.cloud.tencent.com/nodesource/deb/node_$NODE_MAJOR.x nodistro main" | $SUDO tee /etc/apt/sources.list.d/nodesource.list
        ...
        NODE_MAJOR=20
        # 使用国内镜像源
        curl -fsSL https://mirrors.cloud.tencent.com/nodejs-release/setup_$NODE_MAJOR.x | $SUDO bash -
        $INSTALL_CMD nodejs
    else
